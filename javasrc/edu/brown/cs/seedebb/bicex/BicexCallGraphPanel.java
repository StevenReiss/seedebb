...
@Override void handlePopupMenu(JPopupMenu menu,MouseEvent evt)
{
   ...
   if (n != null) {
      BicexEvaluationContext ctx = n.getTimedContext();
      if (ctx != null) {
...
private class Node extends PetalNodeDefault {
   ...
   Node(BicexEvaluationContext ctx) {
      ...
      for_context = ctx;
      Color bkg = BoardColors.getColor(DEFAULT_BACKGROUND_PROP);
      BoardColors.setColors(getComponent(),DEFAULT_BACKGROUND_PROP);
      getComponent().setBackground(bkg);
    }

   BicexEvaluationContext getContext()			{ return for_context; }

   BicexEvaluationContext getTimedContext() {
      if (!call_tree) {
	 long when = getExecution().getCurrentTime();
	 BicexEvaluationContext pctx = BicexCallGraphPanel.this.getContext();
	 if (pctx == for_context) return for_context;
	 BicexEvaluationContext mtch = findInnerContext(BicexCallGraphPanel.this.getContext(),when);
	 if (mtch != null) return mtch;
       }
      return for_context;
    }

   private BicexEvaluationContext findInnerContext(BicexEvaluationContext par,long after) {
      if (par.getInnerContexts() == null) return null;
      for (BicexEvaluationContext ctx : par.getInnerContexts()) {
	 if (ctx.getStartTime() >= after || ctx.getEndTime() >= after) {
	    if (ctx.getMethod().equals(for_context.getMethod())) {
	       return ctx;
	     }
	  }
	 if (ctx.getEndTime() >= after) {
	    BicexEvaluationContext fnd = findInnerContext(ctx,after);
